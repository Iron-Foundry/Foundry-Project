...
**CI (GitHub Actions, `ubuntu-latest`):** api-backend, web-app and discord-server have `.github/workflows/test.yml`; every submodule has `notify-parent.yml`, which fires a `repository_dispatch` at the root so `update-submodules.yml` advances the pointer. Root `.github/workflows/e2e.yml` runs on push to main / dispatch and simply calls `bash run-tests.sh e2e` - GitHub runners have a Docker socket, so it drives the real compose stack rather than runner `services:`. Its only extra step is `bunx playwright install --with-deps chromium`, because `run-tests.sh` installs chromium without `--with-deps`. Submodule-only checkouts skip root-fixture contract tests (guarded by `fixtures/` existence).
