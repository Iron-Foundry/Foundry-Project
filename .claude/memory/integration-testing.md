...
**CI (Forgejo Actions, `codeberg-small`):** each submodule has `.forgejo/workflows/test.yml`; root `.forgejo/workflows/e2e.yml` runs the compose stack on push to main / dispatch. Submodule-only checkouts skip root-fixture contract tests (guarded by `fixtures/` existence).
