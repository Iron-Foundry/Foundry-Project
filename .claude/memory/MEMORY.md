...
- [Per-worker state under gunicorn](per-worker-state-under-gunicorn.md) — api-backend runs 3 workers, so every module-level singleton is three objects; a cross-worker test must build two instances or it proves nothing
