...
- `launcher_is_the_front_door`: an operator-facing job ships as an `Action` in `scripts/launcher/catalog.py`, never as a new root script; declare its `requires`, default a mutating flag to preview, dispatch to a tuned script rather than reimplementing it, and leave machine entry points (CI, Dockerfile, systemd) independent of the launcher. Ruleset: `.claude/rules/launcher-actions.md` (PAG POLICY). Digest: [[root-launcher]].
- `no_invented_names`: name a new command/tool/module for what it DOES, reusing a term already in the repo; never coin a product-style or branded name (`foundry`, `forge`, ...) on your own initiative - if no plain descriptive name fits, ask. Digest: [[no-invented-names]].
...
- `no_artifacts`: never call the `Artifact` tool or load its skills; deliver any HTML mockup/preview/visual comparison as a standalone `.html` file in the repo's `previews/` directory (never the scratchpad - these are kept work products) and hand over the path. Digest: [[no-artifacts]].
...
- `semver_bump_scope`: bump MINOR, PATCH and prerelease tags (alpha/beta/rc) on your own; NEVER bump MAJOR - suggest it and let the user decide when it is time. A test-only change (suite, fixtures, conftest, runners, flake fixes) bumps NOTHING - its changelog entry waits under `## [Unreleased]`. Python modules are bumped ONLY via `uv version --bump <level>` (uv's own semantics), never by hand-editing `pyproject.toml`. Ruleset: `.claude/rules/versioning.md` (PAG POLICY). Digest: [[semver-bump-scope]].
- `bump_at_push_only`: bump the version ONCE, when the accumulated unstaged work is about to be pushed - never per component, feature, or stage. In between, changelog entries accumulate under a single `## [Unreleased]` heading that the bump then renames; never open a new version heading per addition. Ruleset: `.claude/rules/versioning.md` (PAG POLICY). Digest: [[bump-at-push-only]].
...
Standard runner: root `./run-tests.sh {lint|fast|integration|e2e|all}`, or `./run test <lane>` through the launcher (see [[integration-testing]], [[root-launcher]]). Touching a module means running its suite before done (`tests_follow_code`): every change needs `lint` (ruff check + ruff format --check + pyright per Python module, `tsc --noEmit` for web-app); code that changes runtime behavior needs `fast` as well; anything touching DB/Valkey/pubsub or a cross-service seam needs `integration`; changes to the web<->api / discord<->api / api<->runelite interconnects need `e2e`. New code lands with its tests in the same change, never deferred.
